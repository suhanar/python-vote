...
def raise_frame():
    ...
    btn_reg = tk.Button(top,text="Submit",command=lambda:register(top,entry_name,entry_Id),font=("Times New Roman", 8, "bold"),bg="red")
    ...
    top.mainloop()
def register(top,entry_name,entry_id):

    reg_name = entry_name.get()
    reg_id= entry_id.get()
    if(reg_name=='' or reg_id==''):

        mb.showerror('Error','please input both fields')
        raise_frame()
        #lbl = tk.Label(top,text="enter values")
        ...
        try:
            ...
            #lbl = tk.Label(top,text="enter values")
            #lbl.grid(row=0,column=1)
            mb.showerror('Error','value error')
            print("Value error")
            raise_frame()
...
def see_result(lst_lion,lst_tiger,lst_horse):
    ...
    if(sum(lst_lion)>sum(lst_tiger) and sum(lst_lion)>sum(lst_horse)):
        ...
        lbl_lion4 = tk.Label(top,text="Government will decide",font=("Times New Roman", 15, "bold"),bg="red")
